...
namespace App\Http\Controllers\English\Flirtyboy;
...
class OrderController extends AdController {
    ...
    public function show()
    {
        ...
        $user_infos=DB::table('apps_orders')
            ->where('version','flirtyboy')
            ->orderBy('date','desc')
            ->paginate(6);
        return view('Order.show',['user_infos'=>$user_infos,'version'=>'flirtyboy']);
    }
    /**
     * 展示信息总览
     */
    ...
    {
        $appbuy=config('conf.app_buytype.flirtboy');
        $this->originListTwoDays('flirtyboy','','flirtyboy',$appbuy);
    }
    /**
     * 展示充值视图
     */
    public function orderList()
    {
        ...
        $data=$this->originOrderLIst('flirtyboy','','flirtyboy',$search);
