...
namespace App\Http\Controllers\English\Tinderhookup;
...
class OrderController extends AdController {
    ...
    public function show()
    {
        try{
            ...
            $user_infos=DB::table('apps_orders')
                ->where('version','tinderhookup')
                ->orderBy('date','desc')
                ->paginate(6);
            return view('Order.show',['user_infos'=>$user_infos,'version'=>'tinderhookup']);
        }catch (\Exception $e){
            $user_infos=DB::table('apps_orders')
                ->where('version','tinderhookup')
                ->orderBy('date','desc')
    ...
    public function listTwoDays()
    {
        ...
        $this->originListTwoDays('tinderhookup','tinderhookup','tinderhookup',$appbuy);
    }
    /**
     * 展示充值视图
     */
    public function orderList()
    {
        ...
        $data=$this->originOrderLIst('tinderhookup','tinderhookup','',$search);
