...
namespace App\Http\Controllers\Japan\Flirtdating;
...
class OrderController extends AdController {
    ...
    public function show()
    {
        ...
        $user_infos=DB::table('apps_orders')
            ->where('version','flirtdating_japan')
            ->orderBy('date','desc')
            ->paginate(6);
        return view('Order.show',['user_infos'=>$user_infos,'version'=>'flirtdating_japan']);
    }
    public function listTwoDays()
    {
        $appbuy=config('conf.app_buytype.flirtdating');
        $this->originListTwoDays('flirtdating_japan','flirtdating','flirtdating_japan',$appbuy);
    }
    ...
    public function orderList()
    {
        ...
        $data=$this->originOrderLIst('flirtdating_japan','flirtdating','',$search);
